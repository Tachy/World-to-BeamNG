"""Tests for world_to_beamng.forest.vineyard_generator.

Vine rows are forest items (grape_vine: a 5.6 m long row segment, X axis =
row direction). They are aligned in parallel, straight rows - by default
along the fall line (slope gradient) - and follow the terrain slope.
"""
...
def _matrix(instance):
    """
    rotationMatrix row by row. BeamNG reads the model axes as ROWS (X = row 0):
    measured on BeamNG's own vineyards (italy) - there row 0 follows the terrain gradient in 98.5 % of
    the cases, whereas column 0 is negatively correlated.
    """
...
def test_height_sampler_interpolates_bilinearly():
    ...
    heights[:, :] = np.arange(4)[None, :] * 2.0  # rises with x by 2 m per cell
...
def test_row_direction_nearly_flat_below_threshold_uses_long_axis():
    ...
    # 1 % gradient in y is below the 2 % threshold -> longitudinal axis (x), not the fall line (y)
...
def test_row_direction_on_curved_terrain_ignores_gradient_sign_flips():
    # Crest: the gradient points left on the left side, right on the right side - both belong to the x axis
...
def test_all_rows_are_parallel_to_the_fall_line():
    ...
    for instance in instances:
        forward_xy = _matrix(instance)[0, :2]  # first row = model X axis = row direction
...
def test_segments_abut_along_the_row():
    ...
    assert len(xs) >= 5
    # The segment centers lie in the even horizontal grid (segment length, distributed over the row length: at most
    # a few percent deviation); the model is only tilted.
    gaps = np.diff(xs)
...
def _row_extents(instances, y, segment=ROWS["segment_length"]):
    """Extent (x_min, x_max) of the segments of a row (with a gradient toward x the rows run along x)."""
...
def test_rows_run_up_to_the_polygon_boundary():
    # Length 57 m is not a multiple of 5.6 m: the segments are distributed over the row instead of leaving remainders
...
def test_rows_stop_at_the_road_margin_where_a_road_crosses_the_polygon():
    road_edge = box(30, -10, 36, 100)  # road surface straight through the vineyard
    exclusion = build_exclusion_geometry([road_edge], 2.0)  # 2 m distance to the road edge
...
def test_exclusion_zone_keeps_instances_out_e_g_roads():
    road = box(30, -10, 36, 100)  # path straight through the vineyard
...
def test_rotation_matrix_is_a_proper_rotation_and_follows_the_slope():
    ...
    for instance in instances:
        ...
        forward = m[0]
        # Uphill along +x: model X axis points upward with the slope
        assert forward[2] == pytest.approx(math.sin(math.atan(slope)), abs=1e-6)
        assert m[2, 2] > 0.9  # vines stand upright


def test_diagonal_slope_rows_point_along_the_fall_line_and_climb_with_it():
    # Diagonal slope: a transposed matrix would mirror the row direction at the x axis
    # (across the slope) and reverse the tilt (vines would dive into the ground).
    slope = 0.15
...
def test_vertical_axis_stays_upright_on_a_cross_slope():
    # Rows along y, gradient across them (x): no sideways tilt arises - the vine stays upright
    ...
    for instance in instances:
        m = _matrix(instance)
        assert m[2, 2] > 0.95
        assert m[0][2] == pytest.approx(0.0, abs=1e-6)  # along the contour line: no longitudinal tilt
...
def test_multipolygon_parts_get_their_own_row_direction():
    left = box(0, 0, 60, 60)  # falls toward x
    right = box(200, 0, 260, 60)  # (same plane) -> also x, but computed per part
...
def test_exclusion_geometry_unions_buffered_shapes():
    ...
    assert zone.contains(Point(5, 4.5))  # 2.5 m beside the road, within the 3 m buffer
    assert zone.contains(Point(52, 57.5))
    assert not zone.contains(Point(25, 25))


def _two_slopes(x, y):
    """On the left the terrain falls toward x, on the right (from x=100) toward y."""
...
def test_split_by_direction_splits_where_the_fall_line_changes():
    ...
    assert len(blocks) >= 2
    # each block has a uniform fall line: x on the left, y on the right
    for block in blocks:
...
def test_split_by_direction_does_not_split_below_min_area():
    polygon = box(90, 0, 110, 40)  # 800 m², lies exactly on the kink
...
def test_generate_vineyards_stays_inside_the_terrain_bounds():
    # The OSM query extends beyond the terrain; outside there is no elevation data
    # (the heightmap clamps at the edge) - vines would float in the air there.
...
def test_exclusion_geometry_repairs_self_intersecting_polygons():
    bowtie = Polygon([(0, 0), (10, 10), (10, 0), (0, 10)])  # invalid: loop
...
def test_exclusion_far_away_and_partially_overlapping_give_the_same_rows_as_the_full_difference():
    ...
    far = box(1000, 1000, 1100, 1100).buffer(3)  # far away: changes nothing
    assert generate_vineyard_instances(polygon, plane, ROWS, exclusion=far) == generate_vineyard_instances(polygon, plane, ROWS)

    # partially overlapping, complicated zone (many corner points) extending beyond the edge
    ...
    # every instance lies in the remaining area (up to half the segment length of tolerance at the ends)

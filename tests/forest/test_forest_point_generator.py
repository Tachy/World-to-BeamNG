"""Poisson disk sampling and JSONL output of the forest pipeline (behavior stays the same across speed refactorings)."""
...
def test_density_scales_the_spacing():
    ...
    sparse = _generator().generate_points(polygon, tree_density=0.25)  # spacing 10 m
    dense = _generator().generate_points(polygon, tree_density=1.0)  # spacing 5 m

    assert _min_pair_distance(sparse) >= 10.0 - 1e-9
    assert 3.0 < len(dense) / len(sparse) < 5.0  # ~4x more points at half the spacing
...
def test_packing_is_dense_like_bridson():
    ...
    assert 0.55 < per_cell < 0.85  # Poisson disk fills about 0.7 points per r²
...
def test_forest_json_is_written_as_jsonl_with_ctxid(tmp_path):
    ...
    lines = text.split("\n")
    assert lines[-1] == "" and len(lines) == 3  # one line per instance, trailing newline
    assert lines[0] == '{"type":"oak","pos":[1.5,2.25,3.0],"rotationMatrix":[1,0,0,0,1,0,0,0,1],"scale":1.1,"ctxid":0}'
    assert json.loads(lines[1])["type"] == "grüne_birke"  # no ASCII escaping
...
def test_thin_diagonal_polygon_is_filled_at_the_normal_density():
    # 1500 m long, 12 m wide diagonal: the bounding box (1060 x 1060 m) is more than 99 % empty

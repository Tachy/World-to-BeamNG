...
# --- Regression: Galerie-Verlängerung + Terrain-Einbettung ----------------------------------------------


def test_gallery_extension_zone_is_now_embedded_too():
    """Regression für die "Peaks vor den Galeriepfosten"/"Gelände in der Galerie sichtbar"-Meldung:
    extend_gallery_centerline_ends() verlängerte bisher nur die Centerline, NICHT das road_polygon (mit
    dem embed_roads_into_heightmap() per Punkt-in-Polygon-Test prüft, welche Zellen eingebettet werden).
    Das Galerie-Mesh (Boden/Wand/Dach) wird aus der verlängerten Centerline gebaut und sitzt daher in den
    letzten `extension_m` Metern jedes Endes über UNBEHANDELTEM, natürlichem Gelände (Peaks blieben
    stehen, bergseitig kein Wand-Anschluss ans abgesenkte Terrain).

    Siehe auch tests/geometry/test_road_structures.py::
    test_with_an_osm_mapper_road_polygon_is_rebuilt_to_cover_the_extended_centerline (prüft nur das
    Polygon selbst, hier der direkte Beleg am Heightmap-Ergebnis)."""
    from shapely.geometry import LineString

    from world_to_beamng.geometry.road_structures import extend_gallery_centerline_ends

    class FakeMapper:
        def get_road_properties(self, tags):
            return {"width": 6.0}

    # road_polygon exakt wie in terrain_workflow.py::process_tile() gebaut: gepuffert um die
    # UNVERLÄNGERTE Centerline, cap_style=2 (flach).
    original_centerline = [(0.0, 10.0, 100.0), (20.0, 10.0, 100.0)]
    line = LineString([(x, y) for x, y, z in original_centerline])
    road_polygon = np.array(line.buffer(3.0, cap_style=2).exterior.coords[:-1])
    gallery = {
        "road_id": 1,
        "structure_type": "gallery",
        "trimmed_centerline": np.array(original_centerline, dtype=float),
        "road_polygon": road_polygon,
        "osm_tags": {},
    }

    extended = extend_gallery_centerline_ends([gallery], extension_m=2.0, osm_mapper=FakeMapper())

    size = 30
    origin_x, origin_y, square_size = -5.0, 0.0, 1.0

    # Peak in der 2m-Verlängerungszone (x=-2, jenseits des ursprünglichen Centerline-Endpunkts x=0,
    # aber innerhalb der verlängerten Reichweite) - muss jetzt eingebettet (auf ~100 gesetzt) werden.
    heights_ext = np.full((size, size), 100.0)
    heights_ext[10, 3] = 150.0  # x = origin_x + 3*square_size = -2
    result_ext = embed_roads_into_heightmap(heights_ext, origin_x, origin_y, square_size, extended)
    assert result_ext[10, 3] == pytest.approx(100.0, abs=1e-6)

    # Gegenprobe: eine Zelle weit innerhalb des ursprünglichen (unverlängerten) Polygons muss wie
    # bisher eingebettet werden (kein Rückschritt).
    heights_mid = np.full((size, size), 100.0)
    heights_mid[10, 10] = 150.0  # x = origin_x + 10*square_size = 5
    result_mid = embed_roads_into_heightmap(heights_mid, origin_x, origin_y, square_size, extended)
    assert result_mid[10, 10] == pytest.approx(100.0, abs=1e-6)

"""
Fenster-Atlas: Sprites für Fenster, Türen und Kellerfenster.

Die Sprites sind eigene, kleine Flächen, die 3 cm vor der Putzwand liegen (siehe FacadeMapper). Jedes Sprite hat seine
echte Größe in Metern (Auflösung config.FACADE_WINDOW_ATLAS_PX_PER_M), es gibt keine Zellen und keinen Putz im Sprite.
Bauteil-Relief (Laibung, Rahmen, Sohlbank) steckt in der Normalmap.
"""
...
WINDOW_ATLAS_VERSION = 2  # bei Änderungen an den Sprites erhöhen: erzwingt Neuerzeugen der Dateien
...
_EDGE_PAD_PX = 2  # Rand, in dem Normalmap/Hohlraum-Verdunklung nicht an die Sprite-Kante "wrappen"
...
# (Breite, Höhe) in Metern. Fenster inkl. Sohlbank unten; mit Fensterläden ist das Sprite breiter.
...
class WindowAtlasLayout:
    """Packt die Sprites in Reihen und rechnet Sprite -> Pixel-/UV-Rechteck um."""
...
class _Canvas:
    ...
    def slats(self, box, count: int, color: RGB, depth: float) -> None:
        """Waagerechte Lamellen (Fensterladen)."""
        rows, cols = self._slice(*box)
    ...
    def bars(self, box, count: int, color: RGB) -> None:
        ...
        inner = ((np.round(span) >= 1) & (np.round(span) <= count - 1))[None, :]  # Stäbe nur zwischen den Rändern
...
class WindowAtlasGenerator:
    """Zeichnet den Fenster-Atlas (Albedo, Normalmap, Roughness)."""
    ...
    def generate(self) -> Dict[str, np.ndarray]:
        """
        Returns:
            {"albedo", "normal", "roughness"}: uint8-RGB-Bilder (height_px, width_px, 3)
        """
        ...
        for sprite in WindowSprite:
            ...
            for target, image in zip((albedo, normal, roughness), content):
                # Gutter mit der Randfarbe füllen: bilineare Filterung/Mips sehen nur das Sprite selbst
    ...
    def _window(c: _Canvas, transom: bool, shutters) -> None:
        ...
        c.rect((0, 0, c.w_m, top), color=_REVEAL, height=0.0)  # nichts unter dem Sprite scheint durch
        ...
        c.rect((x0, 0.06, x1, top), color=_REVEAL, height=-6.0)  # Laibung
        c.rect((x0, 0.0, x1, 0.06), color=_SILL, height=2.0, rough=0.7)  # Sohlbank
        c.rect((x0 + 0.04, 0.10, x1 - 0.04, top - 0.04), color=_FRAME, height=-4.0, rough=0.5)  # Rahmen
        glass = (x0 + 0.09, 0.15, x1 - 0.09, top - 0.09)
        c.glass(glass, height=-6.5)
        mid = (x0 + x1) / 2
        c.rect((mid - 0.025, 0.15, mid + 0.025, top - 0.09), color=_FRAME, height=-4.5, rough=0.5)  # Stulp
        if transom:
            bar = 0.15 + 0.70 * (top - 0.09 - 0.15)
            c.rect((x0 + 0.09, bar - 0.025, x1 - 0.09, bar + 0.025), color=_FRAME, height=-4.5, rough=0.5)  # Kämpfer

    @staticmethod
    def _door(c: _Canvas, leaf: RGB, glazed: bool) -> None:
        w, h = c.w_m, c.h_m
        dark = tuple(int(v * 0.6) for v in _REVEAL)
        c.rect((0, 0, w, h), color=dark, height=-7.0)  # Laibung
        c.rect((0.04, 0.0, w - 0.04, h - 0.04), color=_FRAME, height=-5.0, rough=0.5)  # Zarge
        c.rect((0.09, 0.0, w - 0.09, h - 0.09), color=leaf, height=-4.0, rough=0.55)  # Türblatt
        shade = tuple(int(v * 0.86) for v in leaf)
        c.rect((0.17, 0.12, w - 0.17, 0.95), color=shade, height=-5.0)  # Füllung unten
        if glazed:
            c.glass((0.17, 1.15, w - 0.17, h - 0.25), height=-5.5)
        else:
            c.rect((0.17, 1.15, w - 0.17, h - 0.25), color=shade, height=-5.0)
        c.rect((w - 0.21, 0.98, w - 0.16, 1.08), color=(200, 190, 150), lift=2.0, rough=0.3)  # Griff

    @staticmethod
    def _basement(c: _Canvas, bars: bool) -> None:
        w, h = c.w_m, c.h_m
        c.rect((0, 0, w, h), color=_REVEAL, height=-6.0)
        c.rect((0.03, 0.03, w - 0.03, h - 0.03), color=(210, 210, 206), height=-4.0, rough=0.6)  # Rahmen
    ...
    def _clock(c: _Canvas) -> None:
        """Turmuhr: dunkle Tafel mit goldenem Rahmen, Ring, Stundenstrichen und Zeigern (10:10 Uhr)."""
        rows, cols = np.indices((c.h, c.w))
        x = (cols + 0.5) / c.ppm - c.w_m / 2  # Meter nach rechts vom Mittelpunkt
        y = c.h_m / 2 - (rows + 0.5) / c.ppm  # Meter nach oben
        radius = np.hypot(x, y)
        angle = np.arctan2(x, y)  # im Uhrzeigersinn ab 12 Uhr
        ...
        for hour in range(12):
            ...
            offset = np.abs((angle - centre + np.pi) % (2 * np.pi) - np.pi) * radius  # Querabstand in Metern
            quarter = hour % 3 == 0
    ...
    def _finish(albedo: np.ndarray, height: np.ndarray, rough: np.ndarray):
        """Normalmap und Hohlraum-Verdunklung; am Rand erst erweitern, damit nichts um das Sprite herum "wrappt"."""
